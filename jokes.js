...
//console.log(jokesObject.jokes.length)

let randomJokeId = Math.floor(Math.random()*164)

//console.log(jokesObject.jokes[randomJokeId])

let jokesJSON = JSON.stringify(jokesObject.jokes.map(joke=>joke.replace(/\n{2,}/g, '').replace(/A:/g, '\nA:')));

fs.writeFile('jokes.json', jokesJSON, function (err) {
  if (err) throw err;
  console.log('Saved!');
});
